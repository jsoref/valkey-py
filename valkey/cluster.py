...
def get_connection(valkey_node, *args, **options):
...
def cleanup_kwargs(**kwargs):
    ...
    connection_kwargs = {
        ...
        if k in VALKEY_ALLOWED_KEYS and k not in KWARGS_DISABLED_KEYS
    }
...
class ValkeyCluster(AbstractValkeyCluster, ValkeyClusterCommands):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """
         Initialize a new ValkeyCluster client.
         ...
         :param require_full_coverage:
            ...
            commands. See -
            https://valkey.io/topics/cluster-tutorial#valkey-cluster-configuration-parameters
            When set to True: all slots must be covered to construct the
            cluster client. If not all slots are covered, ValkeyClusterException
            will be thrown.
        :param read_from_replicas:
         ...
         :param dynamic_startup_nodes:
             Set the ValkeyCluster's startup nodes to all of the discovered nodes.
        ...
        if url is not None:
            ...
            raise ValkeyClusterException(
                "ValkeyCluster requires at least one node to discover the "
                "cluster. Please provide one of the followings:\n"
                "1. host and port, for example:\n"
                " ValkeyCluster(host='localhost', port=6379)\n"
                "2. list of startup nodes, for example:\n"
                " ValkeyCluster(startup_nodes=[ClusterNode('localhost', 6379),"
                " ClusterNode('localhost', 6378)])"
            )
        # Update the connection arguments
        # Whenever a new connection is established, ValkeyCluster's on_connect
        # method should be run
        # If the user passed on_connect function we'll save it and run it
        # inside the ValkeyCluster.on_connect() function
        self.user_on_connect_func = kwargs.pop("valkey_connect_func", None)
        kwargs.update({"valkey_connect_func": self.on_connect})
    ...
    def disconnect_connection_pools(self):
        for node in self.get_nodes():
            if node.valkey_connection:
                try:
                    node.valkey_connection.connection_pool.disconnect()
                except OSError:
    ...
    def get_valkey_connection(self, node):
        if not node.valkey_connection:
            with self._lock:
                if not node.valkey_connection:
                    self.nodes_manager.create_valkey_connections([node])
        return node.valkey_connection
    ...
    def set_retry(self, retry: "Retry") -> None:
        self.retry = retry
        for node in self.get_nodes():
            node.valkey_connection.set_retry(retry)
    ...
    def monitor(self, target_node=None):
        ...
        if target_node.valkey_connection is None:
            raise ValkeyClusterException(
                f"Cluster Node {target_node.name} has no valkey_connection"
            )
        return target_node.valkey_connection.monitor()
    ...
    def pipeline(self, transaction=None, shard_hint=None):
        ...
        """
        if shard_hint:
            raise ValkeyClusterException("shard_hint is deprecated in cluster mode")

        if transaction:
            raise ValkeyClusterException("transaction is deprecated in cluster mode")
    ...
    def _get_command_keys(self, *args):
        ...
        """
        valkey_conn = self.get_default_node().valkey_connection
        return self.commands_parser.get_keys(valkey_conn, *args)
    ...
    def determine_slot(self, *args):
        ...
        if command.upper() in ("EVAL", "EVALSHA"):
            ...
            if len(args) <= 2:
                raise ValkeyClusterException(f"Invalid args in command: {args}")
            num_actual_keys = int(args[2])
            eval_keys = args[3 : 3 + num_actual_keys]
            # if there are 0 keys, that means the script can be run on any node
            # so we can just return a random slot
            if len(eval_keys) == 0:
                return random.randrange(0, VALKEY_CLUSTER_HASH_SLOTS)
            keys = eval_keys
        else:
            ...
            if keys is None or len(keys) == 0:
                ...
                if command.upper() in ("FCALL", "FCALL_RO"):
                    return random.randrange(0, VALKEY_CLUSTER_HASH_SLOTS)
                raise ValkeyClusterException(
                    "No way to dispatch this command to Valkey Cluster. "
    ...
    def _execute_command(self, target_node, *args, **kwargs):
        ...
        asking = False
        moved = False
        ttl = int(self.ValkeyClusterRequestTTL)
        ...
        while ttl > 0:
            ...
            try:
                ...
                valkey_node = self.get_valkey_connection(target_node)
    ...
    def close(self):
        try:
            ...
            # ValkeyCluster's __init__ can fail before nodes_manager is set
            pass
...
class ClusterNode:
    def __init__(self, host, port, server_type=None, valkey_connection=None):
        if host == "localhost":
        ...
        self.valkey_connection = valkey_connection
    ...
    def __repr__(self):
        return (
            ...
            f"valkey_connection={self.valkey_connection}]"
        )
    ...
    def __del__(self):
        if self.valkey_connection is not None:
            self.valkey_connection.close()

    def flush_cache(self):
        if self.valkey_connection is not None:
            self.valkey_connection.flush_cache()

    def delete_command_from_cache(self, command):
        if self.valkey_connection is not None:
            self.valkey_connection.delete_command_from_cache(command)

    def invalidate_key_from_cache(self, key):
        if self.valkey_connection is not None:
            self.valkey_connection.invalidate_key_from_cache(key)
...
class NodesManager:
    ...
    def create_valkey_connections(self, nodes):
        """
        This function will create a valkey connection to all nodes in :nodes:
        """
        for node in nodes:
            if node.valkey_connection is None:
                node.valkey_connection = self.create_valkey_node(
                    host=node.host, port=node.port, **self.connection_kwargs
                )

    def create_valkey_node(self, host, port, **kwargs):
        if self.from_url:
            # Create a valkey node with a costumed connection pool
            kwargs.update({"host": host})
            kwargs.update({"port": port})
            r = Valkey(connection_pool=self.connection_pool_class(**kwargs))
        else:
            r = Valkey(host=host, port=port, **kwargs)
        return r
    ...
    def _get_or_create_cluster_node(self, host, port, role, tmp_nodes_cache):
        ...
        if target_node is None:
            ...
            if target_node is None or target_node.valkey_connection is None:
    ...
    def initialize(self):
        """
        Initializes the nodes cache, slots cache and valkey connections.
        :startup_nodes:
        ...
        if not startup_nodes_reachable:
            raise ValkeyClusterException(
                f"Valkey Cluster cannot be connected. Please provide at least "
                f"one reachable node: {str(exception)}"
            ) from exception

        # Create Valkey connections to all nodes
        self.create_valkey_connections(list(tmp_nodes_cache.values()))
        ...
        if not fully_covered and self._require_full_coverage:
            ...
            raise ValkeyClusterException(
                ...
                f"{len(tmp_slots)} of {VALKEY_CLUSTER_HASH_SLOTS} "
                f"covered..."
            )
...
class ClusterPubSub(PubSub):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        :type valkey_cluster: ValkeyCluster
        :type node: ClusterNode
        :type host: str
        :type port: int
        """
        self.node = None
        self.set_pubsub_node(valkey_cluster, node, host, port)
        connection_pool = (
            None
            if self.node is None
            else valkey_cluster.get_valkey_connection(self.node).connection_pool
        )
        self.cluster = valkey_cluster
        self.node_pubsub_mapping = {}
        self._pubsubs_generator = self._pubsubs_generator()
        super().__init__(
            connection_pool=connection_pool,
            encoder=valkey_cluster.encoder,
    ...
    def set_pubsub_node(self, cluster, node=None, host=None, port=None):
        ...
        ValkeyClusterException will be thrown if the passed node does not exist
        in the cluster.
        If host is passed without port, or vice versa, a DataError will be
        thrown.
        :type cluster: ValkeyCluster
        :type node: ClusterNode
        :type host: str
        :type port: int
        """
    ...
    def _raise_on_invalid_node(self, valkey_cluster, node, host, port):
        """
        Raise a ValkeyClusterException if the node is None or doesn't exist in
        the cluster.
        """
        if node is None or valkey_cluster.get_node(node_name=node.name) is None:
            raise ValkeyClusterException(
    ...
    def execute_command(self, *args):
        ...
        if self.connection is None:
            if self.connection_pool is None:
                ...
                self.node = node
                valkey_connection = self.cluster.get_valkey_connection(node)
                self.connection_pool = valkey_connection.connection_pool
    ...
    def _get_node_pubsub(self, node):
        try:
            ...
            pubsub = node.valkey_connection.pubsub(
    ...
    def get_valkey_connection(self):
        """
        Get the Valkey connection of the pubsub connected node.
        """
        if self.node is not None:
            return self.node.valkey_connection
...
class ClusterPipeline(ValkeyCluster):
    """
    Support for Valkey pipeline
    in cluster mode
    """
    ...
    def _send_cluster_commands(
        ...
        """
        Send a bunch of cluster commands to the valkey cluster.

        `allow_redirections` If the pipeline should follow
        `ASK` & `MOVED` responses automatically. If set
        to false it will raise ValkeyClusterException.
        """
        ...
        try:
            ...
            # release all of the valkey connections we allocated earlier
    ...
    def eval(self):
        """ """
        raise ValkeyClusterException("method eval() is not implemented")

    def multi(self):
        """ """
        raise ValkeyClusterException("method multi() is not implemented")
    ...
    def _execute_transaction(self, *args, **kwargs):
        """ """
        raise ValkeyClusterException("method _execute_transaction() is not implemented")

    def load_scripts(self):
        """ """
        raise ValkeyClusterException("method load_scripts() is not implemented")

    def watch(self, *names):
        """ """
        raise ValkeyClusterException("method watch() is not implemented")

    def unwatch(self):
        """ """
        raise ValkeyClusterException("method unwatch() is not implemented")
...
def block_pipeline_command(name: str) -> Callable[..., Any]:
    ...
    def inner(*args, **kwargs):
        raise ValkeyClusterException(
            ...
            f"when running valkey in cluster mode..."
        )
...
class NodeCommands:
    ...
    def write(self):
        """
        Code borrowed from Valkey so it can be fixed
        """
        connection = self.connection
    ...
    def read(self):
        ...
        for c in self.commands:
            ...
            # connection error raised by valkey-py.
            # but valkey-py doesn't check in parse_response
